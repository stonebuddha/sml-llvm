...
local
    open C
in
type llcontext = (ST_LLVMOpaqueContext.tag, rw) su_obj ptr'
type llmodule = (ST_LLVMOpaqueModule.tag, rw) su_obj ptr'
type lltype = (ST_LLVMOpaqueType.tag, rw) su_obj ptr'
type llvalue = (ST_LLVMOpaqueValue.tag, rw) su_obj ptr'
type lluse = (ST_LLVMOpaqueUse.tag, rw) su_obj ptr'
type llbasicblock = (ST_LLVMOpaqueBasicBlock.tag, rw) su_obj ptr'
type llbuilder = (ST_LLVMOpaqueBuilder.tag, rw) su_obj ptr'
type llmemorybuffer = (ST_LLVMOpaqueMemoryBuffer.tag, rw) su_obj ptr'
end
...
fun toPtrArr (buf : ('a C.ptr, 'b) C.obj C.ptr') (len : 'c C.uint_obj') : ('a C.ptr') array =
  let
      fun iter i =
        let
            val loc = C.Ptr.|+! C.S.ptr (buf, i)
  ...
  in
      Array.tabulate (Word32.toInt $ C.Get.uint' len, iter)
  end
fun toVPtrArr (buf : 'b C.voidptr_obj C.ptr') (len : 'c C.uint_obj') : C.voidptr array =
  let
      fun iter i =
        let
            val loc = C.Ptr.|+! C.S.voidptr (buf, i)
        in
...
fun dupIntArr (arr : int array) : C.rw C.uint_obj C.ptr' =
  let
      val buf = C.alloc' C.S.uint (Word.fromInt $ Array.length arr)
      fun iter (i, num) =
        let
            val loc = C.Ptr.|+! C.S.uint (buf, i)
        in
            C.Set.uint' (C.Ptr.|*! loc, Word32.fromInt num)
        end
      val () = Array.appi iter arr
  in
      buf
...
fun mdkind_id (C : llcontext) (S : string) : int =
  let
      val S' = ZString.dupML' S
  in
      Word32.toInt (F_llvm_mdkind_id.f' (C, S'))
      before
      C.free' S'
  end
...
fun integer_type (C : llcontext) (NumBits : int) : lltype = F_llvm_integer_type.f' (C, Word32.fromInt NumBits)
fun integer_bitwidth (IntegerTy : lltype) : int = Word32.toInt $ F_llvm_integer_bitwidth.f' IntegerTy
...
fun function_type (ReturnTy : lltype) (ParamTys : lltype array) : lltype =
  let
      val ParamTys' = dupPtrArr ParamTys
  in
      F_llvm_function_type.f' (ReturnTy, ParamTys', Word32.fromInt $ Array.length ParamTys)
      before
      C.free' ParamTys'
  end
fun var_arg_function_type (ReturnTy : lltype) (ParamTys : lltype array) : lltype =
  let
      val ParamTys' = dupPtrArr ParamTys
  in
      F_llvm_var_arg_function_type.f' (ReturnTy, ParamTys', Word32.fromInt $ Array.length ParamTys)
      before
      C.free' ParamTys'
  end
...
fun struct_type (C : llcontext) (ElemTys : lltype array) : lltype =
  let
      val ElemTys' = dupPtrArr ElemTys
  in
      F_llvm_struct_type.f' (C, ElemTys', Word32.fromInt $ Array.length ElemTys)
      before
      C.free' ElemTys'
  end
fun packed_struct_type (C : llcontext) (ElemTys : lltype array) : lltype =
  let
      val ElemTys' = dupPtrArr ElemTys
  in
      F_llvm_packed_struct_type.f' (C, ElemTys', Word32.fromInt $ Array.length ElemTys)
      before
      C.free' ElemTys'
  end
...
fun struct_set_body (StructTy : lltype) (ElemTys : lltype array) (Packed : bool) : unit =
  let
      val ElemTys' = dupPtrArr ElemTys
  in
      F_llvm_struct_set_body.f' (StructTy, ElemTys', Word32.fromInt $ Array.length ElemTys, if Packed then 1 else 0)
      before
      C.free' ElemTys'
  end
...
fun array_type (ElemTy : lltype) (ElemCount : int) : lltype = F_llvm_array_type.f' (ElemTy, Word32.fromInt ElemCount)
fun pointer_type (ElemTy : lltype) : lltype = F_llvm_pointer_type.f' ElemTy
fun qualified_pointer_type (ElemTy : lltype) (AddrSpace : int) : lltype = F_llvm_qualified_pointer_type.f' (ElemTy, Word32.fromInt AddrSpace)
fun vector_type (ElemTy : lltype) (ElemCount : int) : lltype = F_llvm_vector_type.f' (ElemTy, Word32.fromInt ElemCount)

fun element_type (Ty : lltype) : lltype = F_llvm_element_type.f' Ty
fun array_length (ArrayTy : lltype) : int = Word32.toInt $ F_llvm_array_length.f' ArrayTy
fun address_space (PtrTy : lltype) : int = Word32.toInt $ F_llvm_address_space.f' PtrTy
fun vector_size (VectorTy : lltype) : int = Word32.toInt $ F_llvm_vector_size.f' VectorTy
...
(*--... Operations on users ................................................--*)
fun operand (Val : llvalue) (I : int) : llvalue = F_llvm_operand.f' (Val, Word32.fromInt I)
fun operand_use (Val : llvalue) (I : int) : lluse = F_llvm_operand_use.f' (Val, Word32.fromInt I)
fun set_operand (Val1 : llvalue) (I : int) (Val2 : llvalue) : unit = F_llvm_set_operand.f' (Val1, Word32.fromInt I, Val2)
...
fun metadata (Val : llvalue) (MDKindId : int) : llvalue option =
  let
      val Res = F_llvm_metadata.f' (Val, Word32.fromInt MDKindId)
  in
      if C.Ptr.isNull' Res then NONE
      else SOME Res
  end
fun set_metadata (Val : llvalue) (MDKindId : int) (MD : llvalue) : unit = F_llvm_set_metadata.f' (Val, Word32.fromInt MDKindId, MD)
fun clear_metadata (Val : llvalue) (MDKindId : int) : unit = F_llvm_clear_metadata.f' (Val, Word32.fromInt MDKindId)
...
fun mdnode (C : llcontext) (ElemVals : llvalue array) : llvalue =
  let
      val ElemVals' = dupPtrArr ElemVals
  in
      F_llvm_mdnode.f' (C, ElemVals', Word32.fromInt $ Array.length ElemVals)
      before
      C.free' ElemVals'
  end
...
fun get_named_metadata (M : llmodule) (Name : string) : llvalue array =
  let
      val Name' = ZString.dupML' Name
      val Len = C.new' C.S.uint
      val Buf = F_llvm_get_namedmd.f' (M, Name', C.Ptr.|&! Len)
  in
      toPtrArr Buf Len
      before
...
fun const_int_of_string (IntTy : lltype) (S : string) (Radix : int) : llvalue =
  let
      val S' = ZString.dupML' S
  in
      F_llvm_const_int_of_string.f' (IntTy, S', Word32.fromInt Radix)
      before
      C.free' S'
  end
...
fun const_array (ElemTy : lltype) (ElemVals : llvalue array) : llvalue =
  let
      val ElemVals' = dupPtrArr ElemVals
  in
      F_llvm_const_array.f' (ElemTy, ElemVals', Word32.fromInt $ Array.length ElemVals)
      before
      C.free' ElemVals'
  end
fun const_struct (C : llcontext) (ElemVals : llvalue array) : llvalue =
  let
      val ElemVals' = dupPtrArr ElemVals
  in
      F_llvm_const_struct.f' (C, ElemVals', Word32.fromInt $ Array.length ElemVals)
      before
      C.free' ElemVals'
  end
fun const_named_struct (StructTy : lltype) (ElemVals : llvalue array) : llvalue =
  let
      val ElemVals' = dupPtrArr ElemVals
  in
      F_llvm_const_named_struct.f' (StructTy, ElemVals', Word32.fromInt $ Array.length ElemVals)
      before
      C.free' ElemVals'
  end
fun const_packed_struct (C : llcontext) (ElemVals : llvalue array) : llvalue =
  let
      val ElemVals' = dupPtrArr ElemVals
  in
      F_llvm_const_packed_struct.f' (C, ElemVals', Word32.fromInt $ Array.length ElemVals)
      before
      C.free' ElemVals'
  end
fun const_vector (ElemVals : llvalue array) : llvalue =
  let
      val ElemVals' = dupPtrArr ElemVals
  in
      F_llvm_const_vector.f' (ElemVals', Word32.fromInt $ Array.length ElemVals)
      before
      C.free' ElemVals'
  end
...
fun const_element (Val : llvalue) (N : int) : llvalue = F_llvm_const_element.f' (Val, Word32.fromInt N)
...
fun const_gep (ConstantVal : llvalue) (Indices : llvalue array) : llvalue =
  let
      val Indices' = dupPtrArr Indices
  in
      F_llvm_const_gep.f' (ConstantVal, Indices', Word32.fromInt $ Array.length Indices)
      before
      C.free' Indices'
  end
fun const_in_bounds_gep (ConstantVal : llvalue) (Indices : llvalue array) : llvalue =
  let
      val Indices' = dupPtrArr Indices
  in
      F_llvm_const_in_bounds_gep.f' (ConstantVal, Indices', Word32.fromInt $ Array.length Indices)
      before
      C.free' Indices'
  end
...
fun const_extractvalue (Aggregate : llvalue) (Indices : int array) : llvalue =
  ...
  in
      F_llvm_const_extractvalue.f' (Aggregate, Indices', Word32.fromInt $ Array.length Indices)
      before
      C.free' Indices'
  end
fun const_insertvalue (Aggregate : llvalue) (Val : llvalue) (Indices : int array) : llvalue =
  let
      val Indices' = dupIntArr Indices
  in
      F_llvm_const_insertvalue.f' (Aggregate, Val, Indices', Word32.fromInt $ Array.length Indices)
      before
      C.free' Indices'
  end
...
fun alignment (Global : llvalue) : int = Word32.toInt $ F_llvm_alignment.f' Global
fun set_alignment (Bytes : int) (Global : llvalue) : unit = F_llvm_set_alignment.f' (Word32.fromInt Bytes, Global)
...
fun declare_qualified_global (Ty : lltype) (Name : string) (AddrSpace : int) (M : llmodule) : llvalue =
  let
      val Name' = ZString.dupML' Name
  in
      F_llvm_declare_qualified_global.f' (Ty, Name', Word32.fromInt AddrSpace, M)
      before
      C.free' Name'
  end
...
fun define_qualified_global (Name : string) (Initializer : llvalue) (AddrSpace : int) (M : llmodule) : llvalue =
  let
      val Name' = ZString.dupML' Name
  in
      F_llvm_define_qualified_global.f' (Name', Initializer, Word32.fromInt AddrSpace, M)
      before
      C.free' Name'
  end
...
fun global_begin (M : llmodule) : (llmodule, llvalue) llpos =
  let
      val Tag = C.new' C.S.sint
      val Res = F_llvm_global_begin.f' (M, C.Ptr.|&! Tag)
  in
      (case C.Get.sint' Tag of
           0 => At_end $ C.Ptr.cast' Res
         | 1 => Before $ C.Ptr.cast' Res
...
fun global_end (M : llmodule) : (llmodule, llvalue) llrev_pos =
  ...
  in
      (case C.Get.sint' Tag of
           0 => At_start $ C.Ptr.cast' Res
         | 1 => After $ C.Ptr.cast' Res
...
fun iter_global_range f i =
  case i of
      At_end _ => ()
    | Before bb => (f bb; iter_global_range f (global_succ bb))

fun iter_globals f m = iter_global_range f (global_begin m)

fun fold_left_global_range f init i =
  case i of
      At_end _ => init
    | Before bb => fold_left_global_range f (f init bb) (global_succ bb)

fun fold_left_globals f init m = fold_left_global_range f init (global_begin m)

fun rev_iter_global_range f i =
  case i of
      At_start _ => ()
    | After bb => (f bb; rev_iter_global_range f (global_pred bb))

fun rev_iter_globals f m = rev_iter_global_range f (global_end m)

fun fold_right_global_range f i init =
  case i of
      At_start _ => init
    | After bb => fold_right_global_range f (global_pred bb) (f bb init)

fun fold_right_globals f m init = fold_right_global_range f (global_end m) init
...
fun function_call_conv (Fn : llvalue) : int = Word32.toInt $ F_llvm_function_call_conv.f' Fn
fun set_function_call_conv (Id : int) (Fn : llvalue) : unit = F_llvm_set_function_call_conv.f' (Word32.fromInt Id, Fn)
...
fun set_gc (GC : string option) (Fn : llvalue) : unit =
  case GC of
      NONE => F_llvm_set_gc.f' (C.Ptr.null', Fn)
    | SOME S =>
      let
          val S' = ZString.dupML' S
      in
...
fun iter_function_range f i =
  case i of
      At_end _ => ()
    | Before func => (f func; iter_function_range f (function_succ func))

fun iter_functions f m = iter_function_range f (function_begin m)

fun fold_left_function_range f init i =
  case i of
      At_end _ => init
    | Before func => fold_left_function_range f (f init func) (function_succ func)

fun fold_left_functions f init m = fold_left_function_range f init (function_begin m)

fun rev_iter_function_range f i =
  case i of
      At_start _ => ()
    | After func => (f func; rev_iter_function_range f (function_pred func))

fun rev_iter_functions f m = rev_iter_function_range f (function_end m)

fun fold_right_function_range f i init =
  case i of
      At_start _ => init
    | After func => fold_right_function_range f (function_pred func) (f func init)

fun fold_right_functions f m init = fold_right_function_range f (function_end m) init
...
fun param (Fn : llvalue) (Index : int) : llvalue = F_llvm_param.f' (Fn, Word32.fromInt Index)
...
fun iter_param_range f i =
  case i of
      At_end _ => ()
    | Before p => (f p; iter_param_range f (param_succ p))

fun iter_params f func = iter_param_range f (param_begin func)

fun fold_left_param_range f init i =
  case i of
      At_end _ => init
    | Before p => fold_left_param_range f (f init p) (param_succ p)

fun fold_left_params f init func = fold_left_param_range f init (param_begin func)

fun rev_iter_param_range f i =
  case i of
      At_start _ => ()
    | After p => (f p; rev_iter_param_range f (param_pred p))

fun rev_iter_params f func = rev_iter_param_range f (param_end func)

fun fold_right_param_range f i init =
  case i of
      At_start _ => init
    | After p => fold_right_param_range f (param_pred p) (f p init)

fun fold_right_params f func init = fold_right_param_range f (param_end func) init
...
fun set_param_alignment (Arg : llvalue) (Align : int) : unit = F_llvm_set_param_alignment.f' (Arg, Word32.fromInt Align)
...
fun iter_block_range f i =
  case i of
      At_end _ => ()
    | Before bb => (f bb; iter_block_range f (block_succ bb))

fun iter_blocks f func = iter_block_range f (block_begin func)

fun fold_left_block_range f init i =
  case i of
      At_end _ => init
    | Before bb => fold_left_block_range f (f init bb) (block_succ bb)

fun fold_left_blocks f init func = fold_left_block_range f init (block_begin func)

fun rev_iter_block_range f i =
  case i of
      At_start _ => ()
    | After bb => (f bb; rev_iter_block_range f (block_pred bb))

fun rev_iter_blocks f func = rev_iter_block_range f (block_end func)

fun fold_right_block_range f i init =
  case i of
      At_start _ => init
    | After bb => fold_right_block_range f (block_pred bb) (f bb init)

fun fold_right_blocks f func init = fold_right_block_range f (block_end func) init
...
fun iter_instr_range f i =
  case i of
      At_end _ => ()
    | Before i => (f i; iter_instr_range f (instr_succ i))

fun iter_instrs f bb = iter_instr_range f (instr_begin bb)

fun fold_left_instr_range f init i =
  case i of
      At_end _ => init
    | Before i => fold_left_instr_range f (f init i) (instr_succ i)

fun fold_left_instrs f init bb = fold_left_instr_range f init (instr_begin bb)

fun rev_iter_instr_range f i =
  case i of
      At_start _ => ()
    | After i => (f i; rev_iter_instr_range f (instr_pred i))

fun rev_iter_instrs f bb = rev_iter_instr_range f (instr_end bb)

fun fold_right_instr_range f i init =
  case i of
      At_start _ => init
    | After i => fold_right_instr_range f (instr_pred i) (f i init)

fun fold_right_instrs f bb init = fold_right_instr_range f (instr_end bb) init

(*--... Operations on call sites ...........................................--*)
fun instruction_call_conv (Inst : llvalue) : int = Word32.toInt $ F_llvm_instruction_call_conv.f' Inst
fun set_instruction_call_conv (CC : int) (Inst : llvalue) : unit = F_llvm_set_instruction_call_conv.f' (Word32.fromInt CC, Inst)
fun llvm_add_instruction_param_attr (Inst : llvalue) (Index : int) (PA : Int32.int) : unit = F_llvm_add_instruction_param_attr.f' (Inst, Word32.fromInt Index, PA)
fun llvm_remove_instrution_param_attr (Inst : llvalue) (Index : int) (PA : Int32.int) : unit = F_llvm_remove_instruction_param_attr.f' (Inst, Word32.fromInt Index, PA)
...
fun successor (Val : llvalue) (I : int) : llbasicblock = F_llvm_successor.f' (Val, Word32.fromInt I)
fun set_successor (Val : llvalue) (I : int) (BB : llbasicblock) : unit = F_llvm_set_successor.f' (Val, Word32.fromInt I, BB)
fun num_successors (Val : llvalue) : int = Word32.toInt $ F_llvm_num_successors.f' Val
...
fun incoming (PhiNode : llvalue) : (llvalue * llbasicblock) list =
  let
      ...
      val Num = Word32.toInt $ C.Get.uint' Len
      val Arr = toVPtrArr Buf Len before (C.free' Buf; C.free' (C.Ptr.|&! Len))
      fun loop Idx Acc =
        if Idx = 0 then Acc else
        loop (Idx - 2) ((C.Ptr.cast' $ Array.sub (Arr, Idx - 2), C.Ptr.cast' $ Array.sub (Arr, Idx - 1)) :: Acc)
  in
      loop Num []
  end
...
fun position_builder (Pos : (llbasicblock, llvalue) llpos) (B : llbuilder) : unit =
  case Pos of
      At_end BB => F_llvm_position_builder.f' (C.Ptr.inject' BB, 0, B)
    | Before Inst => F_llvm_position_builder.f' (C.Ptr.inject' Inst, 1, B)
...
val build_aggregate_ret : llvalue array -> llbuilder -> llvalue =
 fn RetVals => fn B =>
    let
        val RetVals' = dupPtrArr RetVals
    in
        F_llvm_build_aggregate_ret.f' (RetVals', Word32.fromInt $ Array.length RetVals, B)
        before
        C.free' RetVals'
    end
...
val build_switch : llvalue -> llbasicblock -> int -> llbuilder -> llvalue = fn Of => fn Else => fn EstimatedCount => fn B => F_llvm_build_switch.f' (Of, Else, Word32.fromInt EstimatedCount, B)
...
val build_indirect_br : llvalue -> int -> llbuilder -> llvalue = fn Addr => fn EstimatedDests => fn B => F_llvm_build_indirect_br.f' (Addr, Word32.fromInt EstimatedDests, B)
...
val build_invoke : llvalue -> llvalue array -> llbasicblock -> llbasicblock -> string -> llbuilder -> llvalue =
 fn Fn => fn Args => fn Then => fn Catch => fn Name => fn B =>
    let
        ...
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_invoke.f' (Fn, Args', Word32.fromInt $ Array.length Args, Then, Catch, Name', B)
        before
...
val build_landingpad : lltype -> llvalue -> int -> string -> llbuilder -> llvalue =
 fn Ty => fn PersFn => fn NumClauses => fn Name => fn B =>
    let
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_landingpad.f' (Ty, PersFn, Word32.fromInt NumClauses, Name', B)
        before
        C.free' Name'
    end
...
val build_gep : llvalue -> llvalue array -> string -> llbuilder -> llvalue =
 fn Ptr => fn Indices => fn Name => fn B =>
    let
        ...
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_gep.f' (Ptr, Indices', Word32.fromInt $ Array.length Indices, Name', B)
        before
...
val build_in_bounds_gep : llvalue -> llvalue array -> string -> llbuilder -> llvalue =
 fn Ptr => fn Indices => fn Name => fn B =>
    let
        ...
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_in_bounds_gep.f' (Ptr, Indices', Word32.fromInt $ Array.length Indices, Name', B)
        before
        (C.free' Indices'; C.free' Name')
    end
val build_struct_gep : llvalue -> int -> string -> llbuilder -> llvalue =
 fn Ptr => fn Index => fn Name => fn B =>
    let
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_struct_gep.f' (Ptr, Word32.fromInt Index, Name', B)
        before
        C.free' Name'
    end
...
val build_phi : (llvalue * llbasicblock) list -> string -> llbuilder -> llvalue =
 fn Incoming => fn Name => fn B =>
    let
        ...
        val Vals' = dupPtrArr $ Array.fromList Vals
        val BBs' = dupPtrArr $ Array.fromList BBs
    in
        F_llvm_build_phi.f' (Vals', BBs', Word32.fromInt $ List.length Incoming, Name', B)
        before
...
val build_call : llvalue -> llvalue array -> string -> llbuilder -> llvalue =
 fn Fn => fn Params => fn Name => fn B =>
    let
        val Params' = dupPtrArr Params
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_call.f' (Fn, Params', Word32.fromInt $ Array.length Params, Name', B)
        before
...
val build_extractvalue : llvalue -> int -> string -> llbuilder -> llvalue =
 fn Aggregate => fn Idx => fn Name => fn B =>
    let
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_extractvalue.f' (Aggregate, Word32.fromInt Idx, Name', B)
        before
        C.free' Name'
    end
val build_insertvalue : llvalue -> llvalue -> int -> string -> llbuilder -> llvalue =
 fn Aggregate => fn Val => fn Idx => fn Name => fn B =>
    let
        val Name' = ZString.dupML' Name
    in
        F_llvm_build_insertvalue.f' (Aggregate, Val, Word32.fromInt Idx, Name', B)
        before
        C.free' Name'
    end
...
fun of_string (Name : string option) (Str : string) : llmemorybuffer =
  ...
  in
      (case Name of
           NONE => F_llvm_memorybuffer_of_string.f' (C.Ptr.null', Str')
         | SOME Name =>
           let
               val Name' = ZString.dupML' Name
           in
